(() => {
  ...
  async function preloadImages() {
    const imagesToLoad = CFG.weights.map(w => w.img);
    // 他に必要な画像があればここに追加
    // imagesToLoad.push('./assets/img/other.png');
    ...
    if (btnStartApp) btnStartApp.style.display = 'block';
  }

  function startApp() {
    if (!isLoaded) return;
    
    // Chromeの自動再生制限解除のため、ここで一度再生を叩く
    if (cmVideo) {
      cmVideo.muted = false; // 音を出す
      safePlay(cmVideo);
    }
